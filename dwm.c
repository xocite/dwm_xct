...
static unsigned int scratchtagtwo = 1 << (LENGTH(tags) + 1);

/* compile-time check if all tags fit into an unsigned int bit array. */
struct NumTags { char limitexceeded[LENGTH(tags) > 29 ? -1 : 1]; };
...
manage(Window w, XWindowAttributes *wa)
{
  ...
  selmon->tagset[selmon->seltags] &= ~scratchtagtwo;
	if (!strcmp(c->name, scratchpadnametwo)) {
		c->mon->tagset[c->mon->seltags] |= c->tags = scratchtagtwo;
		c->isfloating = True;
		c->x = c->mon->wx + (c->mon->ww / 2 - WIDTH(c) / 2);
		c->y = c->mon->wy + (c->mon->wh / 2 - HEIGHT(c) / 2);
	}
...
void
togglescratchtwo(const Arg *arg)
{
	Client *c;
	unsigned int found = 0;

	for (c = selmon->clients; c && !(found = c->tags & scratchtagtwo); c = c->next);
	if (found) {
		unsigned int newtagset = selmon->tagset[selmon->seltags] ^ scratchtagtwo;
		if (newtagset) {
			selmon->tagset[selmon->seltags] = newtagset;
			focus(NULL);
			arrange(selmon);
